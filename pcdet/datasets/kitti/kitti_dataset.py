...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        # 读取ImageSets中的对应划分 txt，将帧号存入sample_id_list
        self.sample_id_list = [x.strip() for x in open(split_dir).readlines()] if split_dir.exists() else None

        self.kitti_infos = []
        self.include_kitti_data(self.mode) # 从pkl文件读取数据集信息进self.kitti_infos

    def include_kitti_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading KITTI dataset')
        kitti_infos = [] # 从pkl文件读取数据集信息
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                if count_inside_pts:
                    ...
                    calib = self.get_calib(sample_idx)
                    pts_rect = calib.lidar_to_rect(points[:, 0:3]) # 相机坐标系

                    fov_flag = self.get_fov_flag(pts_rect, info['image']['image_shape'], calib)
                    pts_fov = points[fov_flag] # 只保留视野内的点
                    corners_lidar = box_utils.boxes_to_corners_3d(gt_boxes_lidar) #  边界框转换成角点坐标
                    num_points_in_gt = -np.ones(num_gt, dtype=np.int32) # 初始化为一个 -1 的数组，用于存储每个目标bbox内的点数
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        with open(info_path, 'rb') as f: # kitti_infos_train.pkl
            infos = pickle.load(f)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        # 实际并不会用到这个文件，只是为了初始化 vod_eval.Evaluation
        ...
        out_str_pattern = "Results: \n" + "Entire annotated area: \n" + "Car: {} \n".format(
            str(results['entire_area']['Car_3d_all'])) + "Pedestrian: {} \n".format(
            str(results['entire_area']['Pedestrian_3d_all'])) + "Cyclist: {} \n".format(
            str(results['entire_area']['Cyclist_3d_all'])) + "mAP: {} \n".format(str((results['entire_area'][
                                                                                          'Car_3d_all'] +
                                                                                      results['entire_area'][
                                                                                          'Pedestrian_3d_all'] +
                                                                                      results['entire_area'][
                                                                                          'Cyclist_3d_all']) / 3)) + "Driving corridor area: \n" + "Car: {} \n".format(
            str(results['roi']['Car_3d_all'])) + "Pedestrian: {} \n".format(
            str(results['roi']['Pedestrian_3d_all'])) + "Cyclist: {} \n".format(
            "results['roi']['Cyclist_3d_all']") + "mAP: {} \n".format(str((results['roi']['Car_3d_all'] +
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    print('train file: %s' % train_filename)
    print('val file: %s' % val_filename)
    print('trainval file: %s' % trainval_filename)
    print('test file: %s' % test_filename)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        create_kitti_infos(
            ...
            data_path=ROOT_DIR / 'data' / 'my_radar_5frames',
            save_path=ROOT_DIR / 'data' / 'my_radar_5frames'
        )
